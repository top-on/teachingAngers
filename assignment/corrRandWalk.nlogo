...
globals [
  arrived_turtles ; number of turtles that successfully migrated from habitat i to habitat j
]

to setup
  clear-all
  ;; initialize global veriable
  set arrived_turtles 0
  
  ; draw rings around (0, 0) to give a sense of how far turtles have traveled
  ;; draw the rings by making ring shape turtles that stamp their shape
  ;; and then go away
    crt floor (world-width / 20) [
    set color gray - 1
    set shape "ring"
    set size 20 * (who + 1)
    stamp
  ...
  ;; create turtles and move left, according to 'habitat_distance'
  ;; initial position: 'habitat i'
  crt num-turtles [
    set size 3
  ]
  
  ;; draw target: 'habitat j'
  crt 1 [
    setxy habitat_distance 0
    set size 4
    set shape "circle"
    set color green
    stamp
...
to go
  ;; IF no turtles left: STOP simulation
  if not any? turtles [stop]
  
  ;; Turtles: 1. move, 2. check if at habitat, 3. perhaps die
  ask turtles
  [
    ; head in a random direction
    rt random-normal 0 ((1 - correlation) * 360)
    forward 1
    
    ; check if arrived at habitat (i.e. distance to origin < 2)
    if (distancexy habitat_distance 0) < 2 [
      set arrived_turtles (arrived_turtles + 1)
      die
    ] 
